...
class NCELoss(nn.Module):
    ...
    def forward(self, batch_sample_one, batch_sample_two):
        ...
        print('sim11',sim11.shape)

        m_1=self.FN_mask(sim11)
        m_2 = self.FN_mask(sim12)
        m_3 = self.FN_mask(sim22)
        sim11[m_1 == 1] = float("-inf")
        sim12[m_2 == 1] = float("-inf")
        sim22[m_3 == 1] = float("-inf")
        # mask
...
class MNCELoss(nn.Module):
    """
    Eq. (12): L_{NCE}
    """
    def __init__(self, temperature, device):
        super(MNCELoss, self).__init__()
        self.device = device
        self.criterion = nn.CrossEntropyLoss().to(self.device)
        self.temperature = temperature
        self.cossim = nn.CosineSimilarity(dim=-1).to(self.device)
    ...
    def forward(self, batch_sample_one, batch_sample_two):
        ...
        sim12 = torch.matmul(batch_sample_one, batch_sample_two.t()) / self.temperature
        d = sim12.shape[-1]
        sim11=torch.zeros((d,d)).to(self.device)
        sim22=torch.zeros((d,d)).to(self.device)
        sim11[sim11 == 0]=float("-inf")
        sim22[sim22 == 0] = float("-inf")
        # caculate
        raw_scores1 = torch.cat([sim12, sim11], dim=-1)
        raw_scores2 = torch.cat([sim22, sim12.transpose(-1, -2)], dim=-1)
        logits = torch.cat([raw_scores1, raw_scores2], dim=-2)
