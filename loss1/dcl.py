...
class DCL(object):
    ...
    def __call__(self, z1, z2):
        ...
        inner = torch.mm(z1, z1.t())
        # cross_view1=FN_mask(cross_view_distance)
        #
        # inner1=FN_mask(inner)
        #
        # cross_view_distance[cross_view1==1]= float("-inf")
        # inner[inner1==1]= float("-inf")
        # inner = torch.where(inner >=1, torch.tensor(float("-inf")).cuda(), inner)
        # cross_view_distance = torch.where(cross_view_distance >= 1, torch.tensor(float("-inf")).cuda(), cross_view_distance)



        neg_similarity = torch.cat((inner, cross_view_distance), dim=1) / self.temperature


        neg_mask = torch.eye(z1.size(0), device=z1.device).repeat(1, 2)
        # print('neg_similarity + neg_mask * SMALL_NUM',neg_similarity + neg_mask * SMALL_NUM)
...
class DCLW(DCL):
    ...
    temperature: temperature to control the sharpness of the distribution
    """
    def __init__(self, sigma=0.5, temperature=0.2):
